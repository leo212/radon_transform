...
def direct_radon(image, n):
    ...
    for h in range(0, n):
        ...
        for k in range(0, n/2):
            ...
            radon[h,n/2+k] = ((paddedImage[x1+1,y+1]*w1).sum() + (paddedImage[x2+1,y+1]*w2).sum())
...
def add_to_radon(radon, radonq, r, p, n, rfactors, pfactors, indexes):
    global add_to_radon_time

    start = time.time()
    pairs = np.transpose([np.reshape(r,n*n), np.reshape(np.tile(p,n),n*n)]) # 2s
    valid_pairs = np.where((pairs[:,0]>=0)*(pairs[:,0]<=n/2)*(pairs[:,1]>=0)*(pairs[:,1]<=n/2)) # 1s
    ...
    duration =  (time.time() - start)*1000
    add_to_radon_time+=duration

def merge_radon_squares(radon1, radon2, radon3, radon4, n):
    global merge_time
    ...
    add_to_radon(radon, radon1, r1_ind_low, ps_ind_low, n, r1_ind_low_factor, ps_ind_low_factor, indexes)
    add_to_radon(radon, radon1, r1_ind_low, ps_ind_high, n, r1_ind_low_factor, ps_ind_high_factor, indexes)
    add_to_radon(radon, radon1, r1_ind_high, ps_ind_low, n, r1_ind_high_factor, ps_ind_low_factor, indexes)
    add_to_radon(radon, radon1, r1_ind_high, ps_ind_high, n, r1_ind_high_factor, ps_ind_high_factor, indexes)

    # add four points of top-right quarter
    add_to_radon(radon, radon2, r2_ind_low, ps_ind_low, n, r2_ind_low_factor, ps_ind_low_factor, indexes)
    add_to_radon(radon, radon2, r2_ind_low, ps_ind_high, n, r2_ind_low_factor, ps_ind_high_factor, indexes)
    add_to_radon(radon, radon2, r2_ind_high, ps_ind_low, n, r2_ind_high_factor, ps_ind_low_factor, indexes)
    add_to_radon(radon, radon2, r2_ind_high, ps_ind_high, n, r2_ind_high_factor, ps_ind_high_factor, indexes)

    # add four points of bottom-left quarter
    add_to_radon(radon, radon3, r3_ind_low, ps_ind_low, n, r3_ind_low_factor, ps_ind_low_factor, indexes)
    add_to_radon(radon, radon3, r3_ind_low, ps_ind_high, n, r3_ind_low_factor, ps_ind_high_factor, indexes)
    add_to_radon(radon, radon3, r3_ind_high, ps_ind_low, n, r3_ind_high_factor, ps_ind_low_factor, indexes)
    add_to_radon(radon, radon3, r3_ind_high, ps_ind_high, n, r3_ind_high_factor, ps_ind_high_factor, indexes)

    # add four points of bottom-right quarter
    add_to_radon(radon, radon4, r4_ind_low, ps_ind_low, n, r4_ind_low_factor, ps_ind_low_factor, indexes)
    add_to_radon(radon, radon4, r4_ind_low, ps_ind_high, n, r4_ind_low_factor, ps_ind_high_factor, indexes)
    add_to_radon(radon, radon4, r4_ind_high, ps_ind_low, n, r4_ind_high_factor, ps_ind_low_factor, indexes)
    add_to_radon(radon, radon4, r4_ind_high, ps_ind_high, n, r4_ind_high_factor, ps_ind_high_factor, indexes)

    return radon

# Direct implementation - using loops, slower but more readable
def get_factorized_value(source, x, y, n):
...
def merge_radon_squares_loop(radon1, radon2, radon3, radon4, n):
    min_r_q = -math.sqrt(2*math.pow(n/4,2))
    max_r_q = math.sqrt(2*math.pow(n/4,2))
    min_r = -math.sqrt(2*math.pow(n/2,2))
    max_r = math.sqrt(2*math.pow(n/2,2))
...
def run_two_scale_radon(image, n):
    # load radon 4x4 calculation matrix
    A = sparse.load_npz("direct_radon4x4.npz")
    return two_scale_radon(image, n, A)


def two_scale_radon(image, n, radon4x4Matrix):
    if (n > minSquareSize):
        image1 = image[0:n/2, 0:n/2]
        image2 = image[0:n/2, n/2:n]
        image3 = image[n/2:n, 0:n/2]
        image4 = image[n/2:n, n/2:n]
        radon1 = two_scale_radon(image1, n/2, radon4x4Matrix)
        radon2 = two_scale_radon(image2, n/2, radon4x4Matrix)
        radon3 = two_scale_radon(image3, n/2, radon4x4Matrix)
        radon4 = two_scale_radon(image4, n/2, radon4x4Matrix)
        return merge_radon_squares(radon1, radon2, radon3, radon4, n)
    else:
        # calculate radon using matrix multiplication
        X = np.reshape(image, (minSquareSize * minSquareSize))
        R = radon4x4Matrix * X
        r = np.reshape(R, (minSquareSize, minSquareSize))
        # r = FSS(image, n)
        return r
        #return direct_radon(image, minSquareSize)


def testTwoScale(image):
    image = misc.imread(image, flatten=True).astype('float64')
    n = np.shape(image)[0]

    full_radon = direct_radon(image, n)
    global direct_radon_time
    direct_radon_time = 0

    start = time.time()
    radon0 = run_two_scale_radon(image, n)
    duration =  time.time() - start
    print("2-Scale recursion took:" + str(duration*1000) + "ms")
    print("direct radon times:" + str(direct_radon_time) + "ms")
    print("add_to_radon:" + str(add_to_radon_time) + "ms")
    print("merge_time:" + str(merge_time) + "ms")

    # image1 = image[0:n/2, 0:n/2]
    # image2 = image[0:n/2, n/2:n]
    # image3 = image[n/2:n, 0:n/2]
    # image4 = image[n/2:n, n/2:n]
    # radon1 = two_scale_radon(image1, n/2)
    # radon2 = two_scale_radon(image2, n/2)
    # radon3 = two_scale_radon(image3, n/2)
    # radon4 = two_scale_radon(image4, n/2)

    # Plot the original and the radon transformed image
    plt.subplot(1, 3, 1), plt.imshow(image, cmap='gray')
    plt.xticks([]), plt.yticks([])
    plt.xlabel("Original Image")
    plt.subplot(1, 3, 2), plt.imshow(full_radon, cmap='gray')
    plt.xticks([]), plt.yticks([])
    plt.xlabel("Direct Radon")
    plt.subplot(1, 3, 3), plt.imshow(radon0, cmap='gray')
    plt.xticks([]), plt.yticks([])
    plt.xlabel("2-Scale Radon")
    # plt.subplot(4, 4, 5), plt.imshow(image1, cmap='gray')
    # plt.xticks([]), plt.yticks([])
    # plt.xlabel("Q1")
    # plt.subplot(4, 4, 6), plt.imshow(image2, cmap='gray')
    # plt.xticks([]), plt.yticks([])
    # plt.xlabel("Q2")
    # plt.subplot(4, 4, 7), plt.imshow(image3, cmap='gray')
    # plt.xticks([]), plt.yticks([])
    # plt.xlabel("Q3")
    # plt.subplot(4, 4, 8), plt.imshow(image4, cmap='gray')
    # plt.xticks([]), plt.yticks([])
    # plt.xlabel("Q4")
    # plt.subplot(4, 4, 9), plt.imshow(radon1, cmap='gray')
    # plt.xticks([]), plt.yticks([])
    # plt.xlabel("Radon Q1")
    # plt.subplot(4, 4, 10), plt.imshow(radon2, cmap='gray')
    # plt.xticks([]), plt.yticks([])
    # plt.xlabel("Radon Q2")
    # plt.subplot(4, 4, 11), plt.imshow(radon3, cmap='gray')
    # plt.xticks([]), plt.yticks([])
    # plt.xlabel("Radon Q3")
    # plt.subplot(4, 4, 12), plt.imshow(radon4, cmap='gray')
    # plt.xticks([]), plt.yticks([])
    # plt.xlabel("Radon Q4")

    plt.show()

testTwoScale("phantom128x128.png")
